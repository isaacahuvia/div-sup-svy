...
### Uptake by Discipline

```{r}
# Bivariate
uptake_by_dis <- glm(
  data = model_data,
  formula = div_sup_applied ~ discipline_psych,
  family = "binomial"
)
...
or_glm(model_data, uptake_by_dis) %>%
  kable(digits = 2) %>%
  kable_styling()

# Adjusted
uptake_by_dis_adj <- glm(
  data = model_data,
  formula = as.formula("div_sup_applied ~ discipline_psych + " %+% paste0(researcher_characteristics, collapse = " + ")),
  family = "binomial"
)
...
or_glm(model_data, uptake_by_dis_adj, incr = list(
  age = 10,
  research_time = 10,
  research_funding = 10,
  years_experience = 10
...
success_by_dis <- glm(
  data = model_data,
  formula = div_sup_granted ~ discipline_psych,
  family = "binomial"
)
...
or_glm(model_data, success_by_dis) %>%
  kable(digits = 2) %>%
  kable_styling()

# Adjusted
success_by_dis_adj <- glm(
  data = model_data,
  formula = as.formula("div_sup_granted ~ discipline_psych + " %+% paste0(researcher_characteristics, collapse = " + ")),
  family = "binomial"
)
...
or_glm(model_data, success_by_dis_adj, incr = list(
  age = 10,
  research_time = 10,
  research_funding = 10,
  years_experience = 10
